...
type CobraArgs struct {
	...
	LogLevel string `cobra:"logger" short:"L" usage:"日志级别: debug|info|warn|error"`
	LogPath  string `cobra:"logger-path" usage:"日志路径 logger path"`
	Proxied  string `cobra:"proxies" short:"P" usage:"本地代理 proxies"`
	MView    bool   `cobra:"models" short:"M" usage:"展示模型列表"`
	Hook     bool   `cobra:"hook" short:"H" usage:"鼠标助手"`
}

func Execute() {
	bindCobraTags(cmd, reflect.ValueOf(cArgs))
